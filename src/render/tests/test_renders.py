...
def list_all_render_test_configs():
    ...
    for variant in mi.variants():
        is_jit = ("cuda" in variant or "llvm" in variant or
                  "metal" in variant or "amd" in variant)
...
def test_render(variant, scene_fname, integrator_type, jit_flags_key):
    ...
    if 'cuda' in variant or 'llvm' in variant or 'metal' in variant or 'amd' in variant:
